...
class MicrogridEnv(gym.Env):
    def __init__(self, data, params,horizon=HORIZON):
        ...
        self.prev_powers = {
            'p_import': 0, 'p_export': 0, 'p_wt': 0, 'p_pv': 0,
            'p_dg': 0, 'p_ch_es': 0, 'p_dis_es': 0
        }
        self.prev_soc_es = 0
    ...
    def _get_obs(self):
        ...
        obs[idx:idx+2] = [safe_param("price_import"), safe_param("price_export")]
        idx += 2
        
        # Cost parameters (cost-related)
        obs[idx:idx+5] = [ safe_param("Cop_ma_wt"), safe_param("Cop_ma_pv"), 
                          safe_param("rho_fuel"), safe_param("C_startup"), safe_param("C_degrad_es")]
        idx += 5
        
        # Efficiency parameters (binary/scalar factors)
        obs[idx:idx+3] = [ safe_param("eta_dg"), safe_param("eta_ch_es"), 
                          safe_param("eta_dis_es")]
        idx += 3
        

        
        # Power capacity limits (power values)
        obs[idx:idx+7] = [safe_param("P_grid_import_max"), safe_param("P_grid_export_max"), 
                          safe_param("PWT_max"), safe_param("PPV_max"), 
        ...
        obs[idx:idx+7] = [self.prev_powers['p_import'], self.prev_powers['p_export'], 
                          self.prev_powers['p_wt'], self.prev_powers['p_pv'], 
                            self.prev_powers['p_dg'], 
        ...
        obs[idx:idx+1] = [self.prev_soc_es]
        idx += 1
        
        # Binary states from previous timestep
        obs[idx:idx+3] = [self.prev_u_dg, self.prev_u_es, self.prev_u_maingrid]
        idx += 3
    ...
    def step(self, action):
        ...
        p_dg = action[3] * p_dg_max
        p_pv = action[4] * p_pv_max
        ...
        self.prev_powers = {
            'p_import': p_import, 'p_export': p_export, 'p_wt': p_wt, 'p_pv': p_pv, 
             'p_dg': p_dg, 'p_ch_es': p_ch_es, 'p_dis_es': p_dis_es, 
        ...
        reward,breakdown = compute_reward(
            ...
            # Power variables
            p_import=p_import, p_export=p_export, p_wt=p_wt, p_pv=p_pv, 
             p_dg=p_dg, p_dis_es=p_dis_es, p_ch_es=p_ch_es,
            # Price parameters
